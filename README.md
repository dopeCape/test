...
- **Feed** — full-screen vertical snap-scroll. One note per card. Weighted
  shuffle so high-priority reminders resurface more often.
- **Notes** — CRUD for notes with categories (`remind-me`, `tech`, `ai`,
  `papers`, `code`, `idea`) and per-note weight.
- **Settings** — toggle which categories appear, change the shuffle seed,
  log out.
